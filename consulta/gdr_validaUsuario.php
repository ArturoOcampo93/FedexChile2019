...
if (isset($_POST['user'])) {
	...
	if($vigencia){
		...
		//contra
		if (!preg_match('/^([a-z0-9 ñáéíóú]{2,60})$/i', $data['password'])) {
      ...
      if($data['user'] == 'admin@fedex.com' && $data['password'] == '20fedex18' ){
				...
				$resultado=array("usuario"=>$data['user'],"nombre"=>'admin');
				$ses = Tocken::nuevoToken(json_encode($resultado));
				//echo $ses;
				$_SESSION['costenaAdmin']=$ses;
			}else{
